...
def save_user(user_id, memory):
    cur = con.cursor()
    cur.execute('INSERT INTO memories(user_id, memory_string) values(?, ?)', (user_id, memory))
    print(f'User : {user_id} inserted succesfully')
    cur.close()

def save_memory(user_id, memory):
    cur = con.cursor()
    cur.execute('UPDATE memories SET memory_string = ? where user_id = ?', (user_id, memory))
...
def get_memory(user_id):
    ...
    memory = res.fetchone()
    print(f'User: {user_id} memory : {memory} succesfully retrieved')
    cur.close()
    return memory
...
if __name__ == '__main__' :
    user_id, memory = '001','Welcome' 
    dbname = 'ChatBot'
    create_table(dbname)
    save_user(user_id, memory)
    save_memory(user_id, 'New Memory')
    print(user_exist(user_id))
    print(get_memory(user_id))
    drop_table()
